...
class Component extends HTMLElement {
    ...
    handleInternalRender() {
        if (this.inner.length > 0 === false) {
            this.inner = lodashEs.cloneDeep(this.innerHTML);
            this.innerHTML = '';
        }
    ...
    attributeChangedCallback(attrName, oldValue, newValue) {
        const oldState = lodashEs.cloneDeep(this.state);
        const oldAttributes = lodashEs.cloneDeep(this.attributes);
        if (window.crabug) {
    ...
    setState(data) {
        ...
        if (typeof data === 'function') {
            this.state = lodashEs.assign(this.state, data(this.state));
        }
        else {
            this.state = lodashEs.assign(this.state, data);
        }
    ...
    shouldRender(oldState, oldAttributes) {
        if (this.shouldComponentUpdate &&
            ...
            const res = this.shouldComponentUpdate(oldState, oldAttributes ? oldAttributes : lodashEs.cloneDeep(this.attributes));
            if (res) {
